...
function App() {
  const cart = useCart();
  const auth = useAuth();
  const orders = useOrders(auth.user.email, auth.user.name);
  const wishlist = useWishlist();

  return (
    <div className="app-shell">
      <Navbar cartCount={cart.totalCount} wishlistCount={wishlist.count} user={auth.user} onLogout={auth.logout} />
      <Routes>
        ...
        <Route path="/account" element={<Account user={auth.user} addAddress={auth.addAddress} selectAddress={auth.selectAddress} removeAddress={auth.removeAddress} />} />
